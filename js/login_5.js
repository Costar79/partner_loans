document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (!authToken) {
        console.warn("No authentication token found. Skipping auto-login.");
        return;
    }    

    if (authToken) {
        console.log("✅ User token found, attempting auto-login...");
        try {
            
        console.log("Checking for user_token:", getCookie("user_token"), localStorage.getItem("user_token"));
    
            const response = await fetch("/server/api/auth.php", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ user_token: authToken }) // Change 'token' to 'user_token'
            });
            ...
            if (data.error) {
                ...
                console.log("✅ Auto-login successful! Redirecting to apply page...");
